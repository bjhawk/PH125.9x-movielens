...
`Recosystem`[^7] is an R wrapper to a very powerful and parallelized implementation of this approach called LIBMF[^8]. Parallelization is very important in this case. As noted above, the dataset we are working 8ith yields a VERY large sparse matrix. For context, our `training` dataset would yield a matrix of 10677 rows by 69878 users which is prohibitively large. LIBMF is designed to work around both of those limitations by computing chunks of the matrix in parallel and recombining them into a final model.
